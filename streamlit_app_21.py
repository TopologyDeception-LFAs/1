...
def service_tags(name: str):
    ...
    foot = ("foot" in n or "feet" in n or "reflexology" in n or
            u.startswith("F(") or " NSF" in f" {u}" or " NSBF" in f" {u}" or
            " NSBLF" in f" {u}" or " WBF" in f" {u}" or " NSHEF" in f" {u}" or
            u.endswith("F"))
    ...
    # NSH (neck/shoulder/head) without negatives
    ...
    with cols[3]:
        if st.button("登记并分配", type="primary"):
            ...
            if t is not None:
                ...
                created = register_customers(service_chosen, arrival_dt, count=int(group_count)) or {"assigned": [], "waiting": []}
...
_preview_time = now()
try:
    if 'time_mode' in locals() and time_mode == "手动输入":
        parts = (manual_time_str or "").strip().split(":")
        hh, mm = int(parts[0]), int(parts[1])
        ss = int(parts[2]) if len(parts)==3 else 0
...
service_obj = next((s for s in st.session_state.services if 'service_chosen' in locals() and s["name"] == service_chosen), None)
        ...
        if st.session_state.assignments:
            ...
            if editable:
                ...
                for _, row in edited.iterrows():
                    cid = row["客户ID"]
                    rec = next((r for r in st.session_state.assignments if r["customer_id"] == cid), None)
                    if rec:
