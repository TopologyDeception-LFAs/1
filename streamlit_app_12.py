...
def service_tags(name: str):
    n = name.lower(); tags = set()
    if any(k in n for k in ["feet", "foot", "reflexology"]): tags.add("FOOT")
...
def assign_customer(service: Dict, arrival: datetime, prefer_employee: Optional[str] = None) -> Optional[Dict]:
    ...
    chosen = emps[0]
    start_time = max(arrival, chosen["next_free"]); end_time = start_time + timedelta(minutes=service["minutes"])
    ...
    st.session_state._customer_seq += 1; chosen["next_free"] = end_time; chosen["served_count"] += 1
    for i, e in enumerate(st.session_state.employees):
        if e["name"] == chosen["name"]: st.session_state.employees[i] = chosen; break
    ...
    with right:
        ...
        if st.session_state.assignments:
            ...
            if rows:
                ...
                per_emp["收款注释"] = per_emp.apply(note, axis=1); per_emp.rename(columns={"employee":"员工","realized":"营业额($)"}, inplace=True) #per_emp.rename(columns={"employee":"员工","realized":"营业额($)"}, inplace=True)
