...
def serialize_state() -> Dict:
    return {
        "employees": [
            {"name": e["name"], "check_in": e["check_in"].isoformat(), "next_free": e["next_free"].isoformat(), "served_count": e["served_count"], "role": e.get("role","正式")}
            for e in st.session_state.employees
        ],
        "services": st.session_state.services,
        "assignments": [
            {**{k:v for k,v in r.items() if k not in ("start","end")}, "start": r["start"].isoformat(), "end": r["end"].isoformat()}
            for r in st.session_state.assignments
        ],
        "waiting": [
            {"customer_id": w["customer_id"], "service": w["service"], "arrival": w["arrival"].isoformat(), "count": w["count"]}
            for w in st.session_state.waiting
        ],
        "reservations": [
            {"id": r["id"], "customer": r["customer"], "service": r["service"], "employee": r["employee"], "start": r["start"].isoformat(), "status": r.get("status","pending")}
...
def service_tags(name: str):
    n = name.lower()
    tags = set()
    if any(k in n for k in ["feet", "foot", "reflexology"]):
        tags.add("FOOT")
    if any(k in n for k in ["neck", "shoulder", "head"]):
        if not any(x in n for x in ["back", "leg", "whole", "cupping", "remedial", "dry needling", "pregnancy", "children", "sciatica", "elbow", "hip"]):
            tags.add("NSH")
    if not tags:
        tags.add("OTHER")
    return tags
...
def can_employee_do(emp: Dict, service: Dict) -> bool:
    ...
    if role == "正式":
        return True
    if role == "新员工-初级":
        return ("NSH" in tags) and ("FOOT" not in tags)
    if role == "新员工-中级":
        return ("NSH" in tags) or ("FOOT" in tags)
    return True
...
def assign_customer(service: Dict, arrival: datetime, prefer_employee: Optional[str] = None) -> Optional[Dict]:
    ...
    record = {"customer_id": st.session_state._customer_seq, "service": service["name"], "minutes": service["minutes"], "employee": chosen["name"], "start": start_time, "end": end_time, "price": service["price"], "status": "进行中" if start_time <= now() < end_time else ("已完成" if end_time <= now() else "排队中"), "pay_cash": 0.0, "pay_transfer": 0.0, "pay_eftpos": 0.0, "pay_voucher": 0.0, "payment_note": ""}
    ...
    st.session_state.assignments.append(record); save_state()
    return record
...
def try_flush_waiting():
    ...
    for item in st.session_state.waiting:
        ...
        for _ in range(item["count"]):
            ...
            if rec is None:
                still.append({"customer_id": item["customer_id"], "service": item["service"], "arrival": item["arrival"], "count": item["count"]-assigned})
                break
            assigned += 1
...
with tab_emp:
    ...
    cols = st.columns(4)
    with cols[0]: emp_name = st.text_input("员工姓名", placeholder="例如：小张 / Lily")
        ...
        if st.button("签到/上班", type="primary"):
            if emp_name:
                if in_mode == "使用当前时间（墨尔本）":
                    ...
                    try:
                        hh, mm, ss = (lambda p: (int(p[0]), int(p[1]), int(p[2]) if len(p)==3 else 0))(manual_ci_str.strip().split(":"))
                ...
                if t is not None:
                    ...
                    if ex:
                        ...
                        st.session_state.employees.append({"name": name, "check_in": t, "next_free": t, "served_count": 0, "role": role})
...
with tab_cus:
    ...
    with st.expander("☎️ 老顾客预约（指定技师/时间/项目）", expanded=False):
        ...
        with v2:
            if st.button("立即应用到期预约", key="btn_apply_resv"):
                apply_due_reservations(); st.success("已处理到期预约。")
    ...
    with cols[3]:
        if st.button("登记并分配", type="primary"):
            if time_mode == "使用当前时间（墨尔本）": t = arrival_time
                ...
                try:
                    parts = manual_time_str.strip().split(":"); hh, mm, ss = int(parts[0]), int(parts[1]), (int(parts[2]) if len(parts)==3 else 0)
    ...
    if st.session_state.waiting:
        ...
        if st.button("尝试为等待队列重新分配"):
            flushed = try_flush_waiting(); st.success(f"已重新分配 {sum(x['count'] for x in flushed)} 位顾客。" if flushed else "暂无可分配的员工空闲。")
    else:
