...
with tab_board:
    ...
    with left:
        ...
        if queued:
            df_q = pd.DataFrame([
                {"客户ID": r["customer_id"], "员工": r["employee"], "项目": r["service"], "开始": fmt(r["start"]), "结束": fmt(r["end"])}
        ...
        if st.session_state.employees:
            ...
            for idx, e in enumerate(rotation):
                status = "空闲" if e["next_free"] <= now() else f"忙碌至 {fmt(e['next_free'])}"
                rows.append({
            ...
            if mins == 0:
                ...
                st.info(f"下一位应接单员工：{nxt['name']}（预计 {mins} 分钟后空闲，{fmt(nxt['next_free'])}）")
        else:
            st.caption("暂无员工签到。")
