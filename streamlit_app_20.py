...
def service_tags(name: str):
    ...
    u = raw.upper()
    tags = set()
    # FOOT: contains feet/foot/reflexology or abbreviations ending with/including F variants
    ...
    # NSH: neck/shoulder/head family without other tags
...
# ===== 时间更正（修改开始时间/可选调整分钟） =====
def update_record_schedule(record_id: int, new_start_time: datetime, new_minutes: Optional[int]=None) -> Optional[str]:
    rec = next((r for r in st.session_state.assignments if r["customer_id"] == record_id), None)
    if not rec: return "未找到该记录"
    emp = rec["employee"]
    minutes = rec["minutes"] if new_minutes is None else int(new_minutes)
    new_end = new_start_time + timedelta(minutes=minutes)
    # 与该员工其它分配冲突（排除自身）
    for a in st.session_state.assignments:
        ...
        if a["employee"] != emp:
            continue
        if not (new_end <= a["start"] or new_start_time >= a["end"]):
            return f"与其自身其它分配（客户ID {a['customer_id']}）冲突 {fmt_t(a['start'])}-{fmt_t(a['end'])}"
    # 与预约冲突
    rsv = next_reservation_block(emp, new_start_time)
    if rsv is not None and (new_end > rsv or new_start_time >= rsv):
        return f"与预约 {fmt_t(rsv)} 冲突"
    # 应用
    rec["start"], rec["end"], rec["minutes"] = new_start_time, new_end, minutes
    if rec["end"] <= now(): rec["status"] = "已完成"
    elif rec["start"] <= now() < rec["end"]: rec["status"] = "进行中"
    else: rec["status"] = "排队中"
    # 重新计算员工轮值/next_free
    def recompute_all_employees():
        ...
        for rx in st.session_state.assignments:
            name = rx["employee"]
            if name not in by_emp: by_emp[name] = {"count": 0, "latest_end": now()}
            by_emp[name]["count"] += 1
            if by_emp[name]["latest_end"] is None or rx["end"] > by_emp[name]["latest_end"]:
                by_emp[name]["latest_end"] = rx["end"]
        for e in st.session_state.employees:
...
with st.sidebar:
    ...
    with st.expander("管理项目（时长/价格）", expanded=False):
        ...
        def _tags_cached(name):
            return ",".join(sorted(list(service_tags(name))))
        preview["tags"] = preview["name"].apply(_tags_cached)
...
with tab_cus:
    ...
    # === 误录删除 / 加时·追加 / 时间更正（搬到登记页） ===
    st.divider()
    st.markdown("#### 误录删除 / 加时 · 追加项目 / 时间更正")
    all_ids_today = [r["customer_id"] for r in st.session_state.assignments]
    delids_quick = st.multiselect("选择要删除的记录（客户ID）", all_ids_today, key="del_assign_ids_in_cus")
    if st.button("删除所选记录（顾客登记页）", disabled=not delids_quick):
        delete_assignments_by_ids(delids_quick); st.success("已删除所选记录，并已重算员工轮值。")

    if all_ids_today:
        target_id_cus = st.selectbox("选择要加时/追加的记录（客户ID）", all_ids_today, key="target_rec_id_cus")
        mode_cus = st.radio("追加方式", ["延长当前服务", "另起一单（紧接着）"], horizontal=True, key="addon_mode_cus")
        extra_minutes_cus = st.number_input("加时/追加时长（分钟）", min_value=5, max_value=180, step=5, value=10, key="addon_minutes_cus")
        as_new_service_cus = None
        if mode_cus == "另起一单（紧接着）":
            as_new_service_cus = st.selectbox("选择追加的项目（可选）", ["仅加时（无项目名）"] + [s["name"] for s in st.session_state.services], key="addon_service_sel_cus")
        price_override_cus = st.text_input("自定义价格（可选，留空则按每分钟单价或项目价）", value="", key="addon_price_cus")
        if st.button("应用加时/追加（顾客登记页）", key="btn_apply_addon_cus"):
            try:
                pid = int(target_id_cus)
                minutes = int(extra_minutes_cus)
                override = float(price_override_cus) if price_override_cus.strip() else None
                if mode_cus == "延长当前服务":
                    err = extend_or_add_on(pid, "extend", minutes, price_override=override)
                else:
                    svc_name = None if (not as_new_service_cus or as_new_service_cus=="仅加时（无项目名）") else as_new_service_cus
    ...
    if all_ids_today:
        st.markdown("##### 时间更正")
        tgt_id_fix = st.selectbox("选择记录（客户ID）", all_ids_today, key="fix_id")
        new_start_str = st.text_input("新的开始时间（HH:MM 或 HH:MM:SS，日期默认今天）", value=now().strftime("%H:%M"), key="fix_start")
        keep_minutes = st.checkbox("保持原分钟数（不改时长）", value=True, key="fix_keep_minutes")
        new_minutes_val = None
        if not keep_minutes:
            new_minutes_val = st.number_input("新时长（分钟）", min_value=1, max_value=300, value=30, step=1, key="fix_minutes")
        if st.button("应用时间更正", key="btn_fix_time"):
            try:
                parts = new_start_str.strip().split(":"); hh, mm, ss = int(parts[0]), int(parts[1]), (int(parts[2]) if len(parts)==3 else 0)
                new_start_dt = datetime.combine(now().date(), dtime(hour=hh, minute=mm, second=ss), tzinfo=TZ)
                err = update_record_schedule(int(tgt_id_fix), new_start_dt, new_minutes_val)
                if err: st.error(f"无法更正：{err}")
                else: st.success("已更新该记录时间。")
            except Exception as e:
                st.error(f"时间格式错误：{e}")
...
_preview_time = None
try:
    if 'time_mode' in locals() and time_mode == "使用当前时间（墨尔本）":
        _preview_time = now()
    else:
...
service_obj = next((s for s in st.session_state.services if s["name"] == (service_chosen if 'service_chosen' in locals() else s["name"])), None)
...
if active:
    st.markdown("#### 进行中")
    df_act_src = sorted(active, key=lambda x: x["end"])
    df_act = pd.DataFrame([{"客户ID": r["customer_id"], "员工": r["employee"], "项目": r["service"], "开始": fmt_t(r["start"]), "结束": fmt_t(r["end"])} for r in df_act_src])
    st.dataframe(df_act, use_container_width=True, height=180)
    sel_del_act = st.multiselect("删除（进行中）客户ID", [r["customer_id"] for r in df_act_src], key="del_ids_active_quick")
    if st.button("删除所选进行中记录", disabled=not sel_del_act):
        delete_assignments_by_ids(sel_del_act); st.success("已删除所选进行中记录。")
if queued:
    st.markdown("#### 排队中（已分配，未开始）")
    df_q_src = sorted(queued, key=lambda x: x["start"])
    df_q = pd.DataFrame([{"客户ID": r["customer_id"], "员工": r["employee"], "项目": r["service"], "开始": fmt_t(r["start"]), "结束": fmt_t(r["end"])} for r in df_q_src])
    st.dataframe(df_q, use_container_width=True, height=180)
    sel_del_q = st.multiselect("删除（排队中）客户ID", [r["customer_id"] for r in df_q_src], key="del_ids_queued_quick")
    if st.button("删除所选排队中记录", disabled=not sel_del_q):
        delete_assignments_by_ids(sel_del_q); st.success("已删除所选排队中记录。")
if st.session_state.waiting:
    st.markdown("#### 等待分配（未指派员工）")
    wait_src = sorted(st.session_state.waiting, key=lambda x: x["arrival"])
    st.dataframe(pd.DataFrame([{"批次客户ID": w["customer_id"], "项目": w["service"]["name"], "人数": w["count"], "到店": fmt_t(w["arrival"])} for w in wait_src]), use_container_width=True, height=180)
    sel_del_wait = st.multiselect("删除等待批次ID", [w["customer_id"] for w in wait_src], key="del_ids_wait_quick")
    if st.button("删除所选等待批次", disabled=not sel_del_wait):
        delete_waiting_by_ids(sel_del_wait); st.success("已删除所选等待批次。")

# -- 看板与提醒（完整版 + 收款编辑 + 营业额统计 + 删除/追加） --
with tab_board:
    st.subheader("实时看板")
    ...
    with left:
        ...
        if active:
            df_act_src = sorted(active, key=lambda x: x["end"])
            df_act = pd.DataFrame([{"客户ID": r["customer_id"], "员工": r["employee"], "项目": r["service"], "开始": fmt_t(r["start"]), "结束": fmt_t(r["end"]), "剩余(分)": max(0, int((r["end"] - now()).total_seconds() // 60))} for r in df_act_src])
            st.dataframe(df_act, use_container_width=True, height=280)
            sel_del_act_board = st.multiselect("删除（进行中）客户ID", [r["customer_id"] for r in df_act_src], key="del_ids_active_board")
            if st.button("删除所选进行中记录（看板）", disabled=not sel_del_act_board):
                delete_assignments_by_ids(sel_del_act_board); st.success("已删除所选进行中记录。")
        else:
            st.caption("暂无进行中的服务。")
        st.markdown("##### 排队中（已分配，未开始）")
        queued = [r for r in st.session_state.assignments if r["status"] == "排队中"]
        if queued:
            ...
            sel_del_q_board = st.multiselect("删除（排队中）客户ID", [r["customer_id"] for r in df_q_src], key="del_ids_queued_board")
            if st.button("删除所选排队中记录（看板）", disabled=not sel_del_q_board):
                delete_assignments_by_ids(sel_del_q_board); st.success("已删除所选排队中记录。")
        else:
            st.caption("暂无排队中的记录。")
        st.markdown("##### 等待分配（未指派员工）")
        if st.session_state.waiting:
...
**加时与追加项目**
- 在“登记顾客/自动分配”或“看板与提醒 → 今日全部记录”中选择一条记录：
