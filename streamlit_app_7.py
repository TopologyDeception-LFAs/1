...
with tab_emp:
    ...
    with cols[1]:
        # 统一：time_input（步长 60 秒），默认墨尔本当前时间
        check_in_time = st.time_input("签到时间", value=now().time(), step=60)
    with cols[2]:
        if st.button("签到/上班", type="primary"):
            if emp_name:
                # 用墨尔本时区的“今天 + 选定时间”合成 datetime
                t = datetime.combine(now().date(), check_in_time, tzinfo=TZ)
...
**导出与清空**
- 侧边栏可下载今日 CSV 记录，包含客户、员工、时间与价格信息（含日期）。
- “清空今日数据（新一天）”：清空员工、等待队列与当日记录，并重置计数器。
""")
